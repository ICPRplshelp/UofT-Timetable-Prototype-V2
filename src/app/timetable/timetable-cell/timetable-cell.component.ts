...
export class TimetableCellComponent implements OnInit {
  ...
  constructor(private clTimingsSharer: ClTimingsSharerService,
    private selectedCoursesService: SelectedCoursesService,
    private utils: UtilitiesService) { }


  getColor(cd: CourseDisplay): string {
    const targetIndex = this.selectedCoursesService.allCoursesMap.get(cd.courseCode) ?? 0;
    return this.utils.timetableCellColors[targetIndex % this.utils.timetableCellColors.length];
  }
  ...
  isOnline(cd: CourseDisplay): boolean {
    ...
    return !cd.deliveryMode.includes("INPER") && (cd.buildingCode === null || cd.buildingCode === "");
  }
  ...
  minutesToTimestamp(minutesFromMidnight: number): string {
    if (minutesFromMidnight < 0 || minutesFromMidnight >= 1440) {
      throw new Error('Invalid input: minutesFromMidnight should be between 0 and 1439');
    }
